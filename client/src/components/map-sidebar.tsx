...
const MapSidebar: React.FC<SidebarProps> = ({
  ...
  const fetchOrders = async () => {
    const { data } = await axios.get(
      "http://localhost:8080/orders/685cb8d8e2463d42edf37c1e"
    );
    setOrders(data);
  };
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="fixed top-0 left-0 w-[300px] h-full bg-white shadow-xl z-[1003] transition-transform duration-300 ease-in-out transform translate-x-0"
          >
            <Card className="h-full">
              ...
              {orders.map((item, idx) => (
                <CardContent key={idx} className="p-4 border-b">
